...
DOC_DATA_PATH = os.environ.get(
    "ZPINCH_DATA_PATH",
    os.path.join(ROOT_DIR, "source_data", "零维计算结果（对应优化质量）_40MA-60MA_150ns-600ns.doc"),
)
OUTPUT_DIR = os.environ.get("ZPINCH_OUTPUT_DIR", os.path.join(ROOT_DIR, "ai_training_outputs"))
CSV_DATA_PATH = os.environ.get(
    "ZPINCH_CSV_DATA_PATH", os.path.join(ROOT_DIR, "source_data", "zpinch_training_data.csv")
)
REPORT_DOC_PATH = os.environ.get(
    "ZPINCH_REPORT_PATH",
    os.path.join(ROOT_DIR, "零维计算结果（对应优化质量）_40MA-60MA_150ns-600ns_训练报告.doc"),
)
NOTEBOOK_PATH = os.environ.get("ZPINCH_NOTEBOOK_PATH", os.path.join(ROOT_DIR, "zpinch_surrogate_training.ipynb"))
...
def extract_rows_from_doc(doc_path: str) -> np.ndarray:
    ...
    # Prefer the bundled canonical CSV so retraining does not depend on GNU
    # binutils or the legacy DOC container on the destination computer.
    if os.path.exists(CSV_DATA_PATH) and os.environ.get("ZPINCH_PREFER_DOC") != "1":
        rows = np.loadtxt(CSV_DATA_PATH, delimiter=",", skiprows=1, dtype=float)
        if rows.ndim == 1:
            rows = rows.reshape(1, -1)
        if rows.ndim != 2 or rows.shape[1] != 7:
            raise RuntimeError(f"便携 CSV 格式错误: {CSV_DATA_PATH}")
        return rows
...
def zpinch_log_positive(y: np.ndarray) -> np.ndarray:
    """用于 sklearn TransformedTargetRegressor 的正值 log 目标变换。"""
    return np.log(np.maximum(y, 1e-12))


def zpinch_exp_inverse(y: np.ndarray) -> np.ndarray:
    """用于 sklearn TransformedTargetRegressor 的 exp 反变换。"""
    return np.exp(y)


class EnergyToVelocityOutputModel:
    """内部预测 E，外部按速度模型接口输出 v。

    网页 Z-pinch 接口要求速度优先时，model.predict(X) 直接返回 v_cm_per_s。
    这个包装类把更稳定的 log(E) 拟合器封装起来，再用同一物理公式输出速度。
    """

    def __init__(self, energy_model):
        self.energy_model = energy_model

    def fit(self, X: np.ndarray, y_E: np.ndarray):
        self.energy_model.fit(X, y_E)
        return self

    def predict_energy(self, X: np.ndarray) -> np.ndarray:
        return np.maximum(np.asarray(self.energy_model.predict(X), dtype=float), 1e-12)

    def predict(self, X: np.ndarray) -> np.ndarray:
        X_arr = np.asarray(X, dtype=float)
        m_input = X_arr[:, 4]
        return calc_v_from_E_m(self.predict_energy(X_arr), m_input)
...
def main():
    ...
    # 生成 notebook。
    write_notebook_from_script(os.path.abspath(__file__), NOTEBOOK_PATH)
